...
function PageMetrics() {
  ...
  if (loading || !metrics) {
    return (
      <div className="h-[90vh] overflow-y-auto text-white p-8">
        <div className="text-center">Loading Solana transaction metrics...</div>
      </div>
    );
  }
              ...
              {data.recentBlocks.map(function (block, index) {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="pr-4">
                      {block.blockTime
                        ? new Date(block.blockTime * 1000).toLocaleTimeString()
                        : "N/A"}
                    </td>
                    <td>{timeAgo}</td>
                  </tr>
